...
export type BaxieUiAttributeMetadata = {
...
export type BaxieUiMetadata = {
  name: string;
  external_url: string;
  image: string;
  attributes: BaxieUiAttributeMetadata[];
};

export type BaxieUiAttributes = {
  status: string;
  class: string;
  gender: string;
  tail: string;
...
export default class BaxieUi extends Phaser.GameObjects.Container {
  [x: string]: any;
  BaxieUiData:BaxieUiMetadata;
  attributes!: BaxieUiAttributes;
  enemies:BaxieUi[] = [];
  selectedEnemy:BaxieUi|null = null;
  ...
  constructor(scene: any, data: BaxieUiMetadata, x: number, y: number) {
    super(scene, x, y); // Container will be positioned at (x,y)
    this.BaxieUiData = data;
  ...
  renderCharacter() {
    ...
    const nameText = this.scene.add.text(0, -30, this.BaxieUiData.name || "BaxieUi", {
      fontSize: "20px",
      color: "#ffffff",
    }).setOrigin(0.5);
